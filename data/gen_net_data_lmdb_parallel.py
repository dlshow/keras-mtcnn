...
neg_save_dir = r'{0:d}/negative'.format(image_size)
...
for annotation in annotations:
    futures.append(executor.submit(partial(_process_one, annotation)))
a=[future.result() for future in tqdm(futures) if future.result() is not None]
for i in a:
    print(len(i[0]))
pos_cls_list=np.vstack([i[0] for i in a if len(i[0]) > 0])
pos_roi_list=np.vstack([i[1] for i in a if len(i[1]) > 0])
neg_cls_list=np.vstack([i[2] for i in a if len(i[2]) > 0])
part_roi_list=np.vstack([i[3] for i in a if len(i[3]) > 0])


Npos = len(pos_cls_list)
Npart = len(part_roi_list)
Nneg = len(neg_cls_list)

part_keep = npr.choice(Npart, size=min(Npos * 3, Npart), replace=False)  # limit the amount of part sample to keep
neg_keep = npr.choice(Nneg, size=min(Npos*100, Nneg), replace=False)   # limit the amount of negative sample to keep
print('pos:{}\t\tpart:{}\t\tneg:{}'.format(Npos, len(part_keep), len(neg_keep)))
...
os.makedirs('../{0:d}net/{0:d}/'.format(image_size), exist_ok = True)
